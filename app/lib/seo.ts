...
type Meta = React.MetaHTMLAttributes<HTMLMetaElement>;
...
type Metadata = {
  charSet?: LiteralUnion<'utf8', string>;
  title?: string;
  description?: string;
  viewport?: Viewport;
  author?: string;
...
export function createMetadata(metadata: Metadata): Meta[] {
  ...
  if (metadata.viewport) {
    ...
    meta.push({ content: viewport, name: 'viewport' });
  }
  ...
  function addMetaTag(keyType: 'name' | 'property', keyName: string, content?: string) {
    if (typeof content === 'string' && content?.trim() !== '') {
      meta.push({ content, [keyType]: keyName });
    }
  }
